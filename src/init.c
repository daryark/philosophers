...
/*   Updated: 2024/07/01 18:26:52 by dyarkovs         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_philos(t_philosophers *data)
{
	...
	{
		arr[i].l_fork = &data->fork_arr[i % data->n_philos];
		arr[i].r_fork = NULL;
		if (data->n_philos != 1)
			arr[i].r_fork = &data->fork_arr[(i + 1) % data->n_philos];
		arr[i].id = i;
		arr[i].meals_ate = 0;
...
bool init_prog(char **av, t_philosophers *data)
{
	...
	data->prog_start_time = gettimeofday_in_mcrsec();
	printf("prog inited\n");
	return (true);
}