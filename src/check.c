...
/*   Updated: 2024/07/06 20:00:08 by dyarkovs         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	check_dead(t_philosophers *data)
{
	...
	while (++i < data->n_philos)
	{
		...
		if (doesnt_eat_time > data->die_time)
		{
			...
			if (DEBUG)
				debug_print(&data->print_lock, 'd', (int)doesnt_eat_time, (int)data->die_time);
...
bool	check_full_all(t_philosophers *data)
{
	...
	if (n_full == data->n_philos)
	{
		data->stop_prog_flag = true;
		if (DEBUG)
			debug_print(&data->print_lock, 'f', n_full, data->n_philos);
...
int	err_check(int ac, char **av)
{
	...
	[opt:<times to eat>]\n";
	if (ac != 5 && ac != 6)
		return (printf(YELLOW_F "Wrong nubmer of arguments!\n%s" RE, format));
	while (--ac)
	{
		n = ft_atol(av[ac]);
		...
		if (ac == 1 && n > 200)
			return (printf("You shouldn't run with more than 200 philos\n"));
		if (n <= 0)
...
// check_dead_lock for both: check_dead & check_full fns as both
// can change stop_prog_flag
