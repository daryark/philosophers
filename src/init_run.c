...
/*   Updated: 2024/07/02 01:57:57 by dyarkovs         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	create_threads(t_philosophers *data)
{
	...
	while (++i < data->n_philos)
	{
		...
		printf(GREEN "PHILO\nphilo->id: %d\nphilo->fork2: %p (right philo fork)\nphilo->fork1: %p (own fork)\nphilo->thread: %lu\n, philo->meals_ate: %d\n, philo->ate_last_time: %ld\n" RE, data->philo_arr[i].id, data->philo_arr[i].fork2, data->philo_arr[i].fork1, data->philo_arr[i].thread, data->philo_arr[i].meals_ate, data->philo_arr[i].ate_last_time);
	}
	printf("threads done\n");
	return (true);
}

static void	assign_forks(t_philo *philo, t_fork *f1, t_fork *f2)
{
	philo->fork1 = f1;
	philo->fork2 = f2;
}
...
void	init_philos(t_philosophers *data)
{
	...
	while (++i < data->n_philos)
	{
		...
		arr[i].fork1 = &data->fork_arr[i % data->n_philos];
		if (data->n_philos == 1)
			arr[i].fork2 = NULL;
		arr[i].id = i + 1;
		arr[i].meals_ate = 0;
